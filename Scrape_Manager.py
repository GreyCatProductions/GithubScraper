...
def process_organization(organization_name: str, clone_directory_path: str, github_gmail, scraper_nr):
    organization = get_organization(organization_name, github_gmail, scraper_nr)
    repos = organization.get_repos(type="all")

    log(scraper_nr, "INFO", f"{repos.totalCount} repos found")
    ...
    if amount_to_skip > 0:
        log(scraper_nr, "INFO", f"Skipping {amount_to_skip} repos as they already exist")
    ...
    for i, repository in enumerate(repos):
        ...
        log(scraper_nr, "INFO", f"Processing {repository.name} ({i}/{repos.totalCount})")
        try:
            ...
            data["issues"].extend(get_formatted_issues(repository, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "issues processed")
            data["branches"].extend(get_formatted_branches(repository, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "branches processed")
            data["contributions"].extend(get_formatted_contributions(repository, organization_name, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "contributions processed")
            data["users"].extend(get_formatted_users(repository, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "users processed")
            data["forks"].extend(get_formatted_forks(repository, organization_name, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "forks processed")
            data["pulls"].extend(get_formatted_pulls(repository, organization_name, github_gmail, scraper_nr))
            #log(scraper_nr,"INFO", "pulls processed")
            data["commits"].extend(get_formatted_commits(repository, organization_name, scraper_nr))
            ...
            log(scraper_nr, "INFO", f"Finished processing {repository.name}. Request cost = {costed_requests}, Left = {end_requests}")
            ...
            log(scraper_nr, "ERROR", "Error on repository: " + repository.name + " " + str(e))
